     MODULE RUQT_Mod
      ...
      real(8), INTENT(OUT) :: trans_results(:,:),current_results(:,:)
     ...
     write(*,*) "Transport Calculated for State ",state_num
      ...
      if(qchem.eqv..true.) then
       ...
       elseif(pyruqt.eqv..true.) then
        write(*,*) 'Using pyRUQT data for this run'
...
!the device
      ...
      do j=1,energy_val
       ...
       do j=1,energy_val
         ...
         trans_results(j,1)=real(energy_list(j))
         trans_results(j,2)=real(transm(j))
       end do
    ...
    do j=1,volt_val
      ...
      do k=1,energy_val
       ...
       do j=1,volt_val
         ...
         current_results(j,1)=real(voltage(j))
         current_results(j,2)=real(current(j))
       end do
      ...
      do k=1,energy_val
       ...
       write(*,*) "***This option was outdated/buggy and has been removed***"
       write(*,*) "***Please use standalone versions of RUQT to access this feature (not recommended)***"
     end if
