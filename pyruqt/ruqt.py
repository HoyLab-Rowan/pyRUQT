...
def estimate_fermi_energy(mf_object, unit='eV'):
  #Extracts Fermi energy as (HOMO+LUMO)/2 from a completed molecular PySCF calculation.
  #Generated by Gemini Pro on 8/11/2026
  mo_energy = mf_object.mo_energy
  mo_occ = mf_object.mo_occ
    
  homo_idx = np.where(mo_occ > 0)[0][-1]
  lumo_idx = np.where(mo_occ == 0)[0][0]
    
  fermi_hartree = (mo_energy[homo_idx] + mo_energy[lumo_idx]) / 2.0
...
def esc_pyscf_pbc(geofile,dft_functional,basis_set,ecp,lattice_v,meshnum,cell_dim,kpts,pyscf_settings,pyscf_conv_settings):
 ...
 print ('h_scf:',h_scf)
 e_fermi = pbc_elec.fermi
 fermi_en = e_fermi * 27.2114
...
# print scf and mcscf(if casscf is used) orbitals to molden files for visualization
 ...
 while atom_num < num_elec_atoms: 
  ...
  if ao_index == ao_data_len:
   ...
   break
 elec_orb=ao_index-1
 fermi_en = estimate_fermi_energy(pyscf_elec)
